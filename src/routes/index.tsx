...
import { ArrowDown, ArrowRight, Clock3, MapPin, Menu, Moon, Share2, Shirt, Sparkles, Users, Volume2 } from "lucide-react";
...
function Index() {
  ...
  return <div className="bg-background pb-20 text-foreground lg:pb-0">
      ...
      <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-9 pt-28 sm:px-6 sm:pb-14 lg:px-8">
        <div className="max-w-4xl">
          ...
          <div className="mt-6 grid max-w-3xl gap-px bg-bone/20 sm:grid-cols-3">
            {[ ["31 October 2026", Clock3], ["The Lawns · Nakuru", MapPin], ["4 PM — late", Moon] ].map(([text, Icon]) => <div key={String(text)} className="flex min-h-12 items-center gap-3 bg-background/80 px-4 py-3 text-sm font-bold uppercase tracking-widest text-bone"><Icon className="size-4 text-lavender" />{String(text)}</div>)}
          </div>
    ...
    <section id="experience" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28"><div className="grid gap-12 lg:grid-cols-[.8fr_1.4fr]"><div><p className="text-xs font-bold uppercase tracking-[.3em] text-lavender">What awaits</p><h2 className="mt-3 text-5xl text-bone sm:text-6xl">One night.<br/>A different Nakuru.</h2><p className="mt-5 max-w-md text-lg text-muted-foreground">A sophisticated Halloween gathering shaped by atmosphere, style, sound and the people you bring with you.</p></div><div className="grid gap-px bg-border sm:grid-cols-2">{[
      ["01", "Atmosphere", "A dark, cinematic setting inspired by the Rift after hours.", Sparkles], ["02", "Music", "A nightlife soundtrack. Line-up details will be announced by the organizers.", Volume2], ["03", "Costumes", "The brief is simple: arrive wickedly fabulous.", Shirt], ["04", "Social energy", "Come as a couple or gather a crew of four.", Users],
    ].map(([n,title,copy,Icon]) => <article className="min-h-64 bg-card p-6 sm:p-8" key={String(title)}><div className="flex items-start justify-between"><span className="text-xs text-lavender">{String(n)}</span><Icon className="size-5 text-bone-muted" /></div><h3 className="mt-16 text-3xl text-bone">{String(title)}</h3><p className="mt-3 text-muted-foreground">{String(copy)}</p></article>)}</div></div></section>
    ...
    ].map(([q,a]) => <AccordionItem value={q} key={q}><AccordionTrigger className="py-6 text-left text-lg text-bone hover:no-underline">{q}</AccordionTrigger><AccordionContent className="pb-6 text-base text-muted-foreground">{a}</AccordionContent></AccordionItem>)}</Accordion></div></section>
